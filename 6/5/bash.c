...
void cleanup(char **argv, char *cmd, char *cmd_copy);
...
char **parse_cmd(char *cmd) {
    ...
    while (token != NULL && i < ARG_COUNT) {
        if (strcmp(token, ">") == 0 || strcmp(token, ">>") == 0) {
            argv[i++] = token;  // Add redirection symbol as a separate argument
            token = strtok(NULL, " ");  // Get the next token as the file name
        } else {
            argv[i++] = token;
            token = strtok(NULL, " ");
        }
